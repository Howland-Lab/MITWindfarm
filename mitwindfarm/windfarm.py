...
class AnalyticalAvgReferenceWindfarm:
    ...
    def __call__(
        self, layout: Layout, thrust_spts: list[float], yaw_spts: list[float],
        urated: float, base_windfield: Optional[Windfield] = None, TIamb: float = None
    ) -> WindfarmSolution:
    ...
    def from_partial(self, partial: PartialWindfarmSolution,
                     base_windfield: Optional[Windfield] = None, TIamb: float = None
                     ) -> WindfarmSolution:
        ...
        windfield = self.superposition(base_windfield, [])
        for i, (x, y, z) in partial.layout.iter_downstream():
            wakes[i] = self.wake_model(x, y, z, partial.rotors[i], TIamb=TIamb)
            windfield.add_wake(wakes[i])
